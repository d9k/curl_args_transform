...
export class CurlHeader {
  ...
  constructor(curlHeaderString: string, curlHeaderOptions: CurlHeaderOptions = {}) {
    this.curlHeaderOptions = curlHeaderOptions;

    [this.name, this.value] = curlHeaderString.split(/:(.*)/s).map((el) => trim(el, ' ', null));

    const { replaceAuthTokenWithVariable } = curlHeaderOptions;
    ...
    if (this.lowercaseName === 'authorization') {
        if (this.value.startsWith('Bearer')) {
            ...
            this.authTokenValue = this.value.replace(/^Bearer\s+/, '');
        }
    }
